...
public class ConfigFileManager {
    ...
    static StickerPack fromMethodCall(Context context, MethodCall call){
        ...
        // ✅ Đọc isAnimated từ Flutter và set avoidCache cho animated stickers
        // WhatsApp yêu cầu avoidCache = true cho animated .webp để xử lý đúng
        Boolean isAnimated = call.argument("isAnimated");
        boolean avoidCache = (isAnimated != null && isAnimated);

        StickerPack newStickerPack = new StickerPack(identifier, name, publisher, trayImageFileName, "", publisherWebsite, privacyPolicyWebsite, licenseAgreementWebsite, "1", avoidCache);
    ...
    static void writeConfigFile(Context context, String jsonString){
        ...
        try {
            ...
            if (!tmp.renameTo(target)) {
                FileWriter fallbackWriter = new FileWriter(target);
                fallbackWriter.write(jsonString);
