...
class StickerPackValidator {
    ...
    private static final long ONE_KIBIBYTE = 1024; // 1 KB = 1024 bytes
    ...
    static void verifyStickerPackValidity(@NonNull Context context, @NonNull StickerPack stickerPack) throws InvalidPackException {
        ...
        for (final Sticker sticker : stickers) {
            // Nếu là animated pack (avoidCache=true) thì dùng 500KB, ngược lại luôn dùng 100KB
            boolean isAnimated = stickerPack.avoidCache;
            validateSticker(context, stickerPack.identifier, sticker, isAnimated);
        }
    }

    private static void validateSticker(@NonNull Context context, @NonNull final String identifier, @NonNull final Sticker sticker, boolean isAnimated) throws InvalidPackException {
        ...
        validateStickerFile(context, identifier, sticker.imageFileName, isAnimated);
    }

    private static void validateStickerFile(@NonNull Context context, @NonNull String identifier, @NonNull final String fileName, boolean isAnimated) throws InvalidPackException {
        try {
            final byte[] bytes = StickerPackLoader.fetchStickerAsset(identifier, fileName, context);
            int sizeLimit = isAnimated ? ANIMATED_STICKER_FILE_SIZE_LIMIT_KB : STICKER_FILE_SIZE_LIMIT_KB;
            if (bytes.length > sizeLimit * ONE_KIBIBYTE) {
                throw new InvalidPackException(InvalidPackException.IMAGE_TOO_BIG,"sticker should be less than " + sizeLimit + "KB, sticker pack identifier:" + identifier + ", filename:" + fileName);
            }
            /*try {
